...
export class FamilyAquacul {

  constructor() { }

  select(db: any, args: any): Bluebird {
    return new Bluebird((resolve, reject) => {
      let query = selectByAquaculSearchForm;
      const values = [];
      if (args) {
        let filterModel;
        ...
        if (args.aquacul01Nr) {
          where.push(' TF.AQUACUL01_NR=?');
          values.push(args.aquacul01Nr);
        }
        if (args.aquacul011Nr) {
          where.push(' TF.AQUACUL011_NR=?');
          values.push(args.aquacul011Nr);
        }
        if (args.aquacul02Nr) {
          where.push(' TF.AQUACUL02_NR=?');
          values.push(args.aquacul01Nr);
        }
        if (args.aquacul021Nr) {
          where.push(' TF.AQUACUL021_NR=?');
          values.push(args.aquacul021Nr);
        }
        if (args.aquacul03Nr) {
          where.push(' TF.AQUACUL03_NR=?');
          values.push(args.aquacul03Nr);
        }
        if (args.aquacul031Nr) {
          where.push(' TF.AQUACUL031_NR=?');
          values.push(args.aquacul031Nr);
        }
        if (args.aquacul032Nr) {
          where.push(' TF.AQUACUL032_NR=?');
          values.push(args.aquacul032Nr);
        }
        if (args.aquacul04Nr) {
          where.push(' TF.AQUACUL04_NR=?');
          values.push(args.aquacul04Nr);
        }
        if (args.aquacul05Nr) {
          where.push(' TF.AQUACUL05_NR=?');
          values.push(args.aquacul05Nr);
        }
  ...
  del(db: any, args: any): Bluebird {
    return new Bluebird((resolve, reject) => {
      db.driver.execQuery(deleteByUserIdForTaAquaculList, [args], (err, data) => {
        if (err) {
          reject(err);
        } else {
          resolve(TableUtils.toCamelCase(data));
...
const selectByAquaculSearchForm = `
  SELECT
    TF.*,
    TU.FAMILY_CD,
    TU.USER_ID,
  ...
  FROM
    TA_USER TU
    LEFT OUTER JOIN TA_FAMILYAQUACUL TF
...
const update = 'CALL PW_FAMILYAQUACUL_Upd(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
const deleteByUserIdForTaAquaculList = `
UPDATE
  TA_FAMILYAQUACUL
SET
	DEL_FL = 1
