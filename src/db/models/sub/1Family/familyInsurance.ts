...
export class FamilyInsurance {
  ...
  select(db: any, args: any): Bluebird {
    return new Bluebird((resolve, reject) => {
      let query = selectByInsuranceSearchForm;
      const values = [];
      if (args) {
        let filterModel;
        ...
        if (args.insurance011Fl) {
          where.push(' TF.INSURANCE011_FL=?');
          values.push(args.insurance011Fl);
        }
        if (args.insurance012Nr) {
          where.push(' TF.INSURANCE012_NR=?');
          values.push(args.insurance012Nr);
        }
        if (args.insurance013Nr) {
          where.push(' TF.INSURANCE013_NR=?');
          values.push(args.insurance013Nr);
        }
        if (args.insurance014Tx) {
          where.push(' TF.INSURANCE014_TX=?');
          values.push(args.insurance014Tx);
        }
  ...
  del(db: any, args: any): Bluebird {
    return new Bluebird((resolve, reject) => {
      db.driver.execQuery(deleteByUserIdForTaInsuranceList, [args], (err, data) => {
        if (err) {
          reject(err);
        } else {
          resolve(TableUtils.toCamelCase(data));
...
const selectByInsuranceSearchForm = `
  SELECT
    TF.*,
    TU.FAMILY_CD,
    TU.USER_ID,
  ...
  FROM
    TA_USER TU
    LEFT OUTER JOIN TA_FAMILYINSURANCE TF
...
const update = 'CALL PW_FAMILYINSURANCE_Upd(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
const deleteByUserIdForTaInsuranceList = `
UPDATE
  TA_FAMILYINSURANCE
SET
	DEL_FL = 1
