...
export class FamilyLive {
  ...
  select(db: any, args: any): Bluebird {
    return new Bluebird((resolve, reject) => {
      let query = selectByLiveSearchForm;
      const values = [];
      if (args) {
        let filterModel;
        ...
        if (args.live011Nr) {
          where.push(' TF.LIVE011_NR=?');
          values.push(args.live011Nr);
        }
        if (args.live012Nr) {
          where.push(' TF.LIVE012_NR=?');
          values.push(args.live012Nr);
        }
        if (args.live013Nr) {
          where.push(' TF.LIVE013_NR=?');
          values.push(args.live013Nr);
        }
        if (args.live014Nr) {
          where.push(' TF.LIVE014_NR=?');
          values.push(args.live014Nr);
        }
  ...
  del(db: any, args: any): Bluebird {
    return new Bluebird((resolve, reject) => {
      db.driver.execQuery(deleteByUserIdForTaLiveList, [args], (err, data) => {
        if (err) {
          reject(err);
        } else {
          resolve(TableUtils.toCamelCase(data));
...
const selectByLiveSearchForm = `
  ...
  FROM
    TA_USER TU
    LEFT OUTER JOIN TA_FAMILYLIVE TF
...
const update = 'CALL PW_FAMILYLIVE_Upd(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
const deleteByUserIdForTaLiveList = `
UPDATE
  TA_FAMILYLIVE
SET
	DEL_FL = 1
