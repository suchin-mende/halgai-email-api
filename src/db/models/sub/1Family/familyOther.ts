...
export class FamilyOther {

  constructor() { }

  select(db: any, args: any): Bluebird {
    return new Bluebird((resolve, reject) => {
      let query = selectByOtherSearchForm;
      const values = [];
      if (args) {
        let filterModel;
        ...
        if (args.other011Nr) {
          where.push(' TF.OTHER011_NR=?');
          values.push(args.other011Nr);
        }
        if (args.other012Nr) {
          where.push(' TF.OTHER012_NR=?');
          values.push(args.other012Nr);
        }
        if (args.other021Nr) {
          where.push(' TF.OTHER021_NR=?');
          values.push(args.other021Nr);
        }
        if (args.other022Nr) {
          where.push(' TF.OTHER022_NR=?');
          values.push(args.other022Nr);
        }
        if (args.other023Nr) {
          where.push(' TF.OTHER023_NR=?');
          values.push(args.other023Nr);
        }
        if (args.other024Nr) {
          where.push(' TF.OTHER024_NR=?');
          values.push(args.other024Nr);
        }
        if (args.other025Nr) {
          where.push(' TF.OTHER025_NR=?');
          values.push(args.other025Nr);
        }
        if (args.other026Nr) {
          where.push(' TF.OTHER026_NR=?');
          values.push(args.other026Nr);
        }
        if (args.other027Nr) {
          where.push(' TF.OTHER027_NR=?');
          values.push(args.other027Nr);
        }
        if (args.other028Nr) {
          where.push(' TF.OTHER028_NR=?');
          values.push(args.other028Nr);
        }
  ...
  del(db: any, args: any): Bluebird {
    return new Bluebird((resolve, reject) => {
      db.driver.execQuery(deleteByUserIdForTaOtherList, [args], (err, data) => {
        if (err) {
          reject(err);
        } else {
          resolve(TableUtils.toCamelCase(data));
...
const selectByOtherSearchForm = `
  ...
  FROM
    TA_USER TU
    LEFT OUTER JOIN TA_FAMILYOTHER TF
...
const update = 'CALL PW_FAMILYOTHER_Upd(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
const deleteByUserIdForTaOtherList = `
UPDATE
  TA_FAMILYOTHER
SET
	DEL_FL = 1
