...
export class FamilyShed {

  constructor() { }

  select(db: any, args: any): Bluebird {
    return new Bluebird((resolve, reject) => {
      let query = selectByShedSearchForm;
      const values = [];
      if (args) {
        let filterModel;
        ...
        if (args.shed011Nr) {
          where.push(' TF.SHED011_NR=?');
          values.push(args.shed011Nr);
        }
        if (args.shed012Nr) {
          where.push(' TF.SHED012_NR=?');
          values.push(args.shed012Nr);
        }
        if (args.shed021Nr) {
          where.push(' TF.SHED021_NR=?');
          values.push(args.shed021Nr);
        }
        if (args.shed022Nr) {
          where.push(' TF.SHED022_NR=?');
          values.push(args.shed022Nr);
        }
        if (args.shed031Nr) {
          where.push(' TF.SHED031_NR=?');
          values.push(args.shed031Nr);
        }
        if (args.shed032Nr) {
          where.push(' TF.SHED032_NR=?');
          values.push(args.shed032Nr);
        }
        if (args.shed041Nr) {
          where.push(' TF.SHED041_NR=?');
          values.push(args.shed041Nr);
        }
        if (args.shed042Nr) {
          where.push(' TF.SHED042_NR=?');
          values.push(args.shed042Nr);
        }
        if (args.shed051Nr) {
          where.push(' TF.SHED051_NR=?');
          values.push(args.shed051Nr);
        }
        if (args.shed052Nr) {
          where.push(' TF.SHED052_NR=?');
          values.push(args.shed052Nr);
        }
        if (args.shed061Nr) {
          where.push(' TF.SHED061_NR=?');
          values.push(args.shed061Nr);
        }
        if (args.shed062Nr) {
          where.push(' TF.SHED062_NR=?');
          values.push(args.shed062Nr);
        }
  ...
  del(db: any, args: any): Bluebird {
    return new Bluebird((resolve, reject) => {
      db.driver.execQuery(deleteByUserIdForTaShedList, [args], (err, data) => {
        if (err) {
          reject(err);
        } else {
          resolve(TableUtils.toCamelCase(data));
...
const selectByShedSearchForm = `
  ...
  FROM
    TA_USER TU
    LEFT OUTER JOIN TA_FAMILYSHED TF
...
const update = 'CALL PW_FAMILYSHED_Upd(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
const deleteByUserIdForTaShedList = `
UPDATE
  TA_FAMILYSHED
SET
	DEL_FL = 1
