...
export class FamilyAquacul extends BaseRoute {
  ...
  public static create(router: Router) {
    // add aquaculs route
    router.get('/:lan/v1/:id/aquaculs', auth.auth, async (req: any, res: Response, next: NextFunction) => {
      let query;
      ...
      try {
        ...
        const aquacul = await Db1.familyAquacul.select(db, query);
        const totaljson: any = await Db1.familyAquacul.count(db, query);
    ...
    router.put('/:lan/v1/:id/aquaculs', auth.auth, async (req: any, res: Response, next: NextFunction) => {
      ...
      try {
        const db = await Db1.getSubdb(req.session.db);
        const result = await Db1.familyAquacul.update(db, query);
    ...
    router.post('/:lan/v1/:id/aquaculs/delete', auth.auth, async (req: any, res: Response, next: NextFunction) => {
      if (Object.keys(req.body).length <= 0) {
        return res.status(400).send({ errors: [ErrorUtils.getErrorJson(req.session.user.langTx, 'error_http_body_required_jsondata')] });
      }
      ...
      try {
        ...
        for (let plan of req.body.selectItems) {
          result.push(await Db1.familyAquacul.del(db, plan.userId));
        }
