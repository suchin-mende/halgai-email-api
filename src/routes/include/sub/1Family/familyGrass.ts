...
export class FamilyGrass extends BaseRoute {
  ...
  public static create(router: Router) {
    // add grass route
    router.get('/:lan/v1/:id/grass', auth.auth, async (req: any, res: Response, next: NextFunction) => {
      let query;
    ...
    router.put('/:lan/v1/:id/grass', auth.auth, async (req: any, res: Response, next: NextFunction) => {
    ...
    router.post('/:lan/v1/:id/grass/delete', auth.auth, async (req: any, res: Response, next: NextFunction) => {
      if (Object.keys(req.body).length <= 0) {
        return res.status(400).send({ errors: [ErrorUtils.getErrorJson(req.session.user.langTx, 'error_http_body_required_jsondata')] });
      }
