  ...
  public static create(router: Router) {
    ...
    router.get('/v1/:id/lives', auth.auth, async (req: any, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        const lives = await Db.familyLive.select(db, query);
        const totaljson: any = await Db.familyLive.count(db, query);
    ...
    router.put('/v1/:id/lives', auth.auth, async (req: any, res: Response, next: NextFunction) => {
      ...
      try {
        const db = await Db.getSubdb(req.session.db);
        const result = await Db.familyLive.update(db, query);
    ...
    router.post('/v1/:id/lives/delete', auth.auth, async (req: any, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        for (let plan of req.body.selectItems) {
          result.push(await Db.familyLive.del(db, plan.userId));
        }
