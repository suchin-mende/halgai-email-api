...
export class FamilyLoan extends BaseRoute {
  ...
  public static create(router: Router) {
    // add loans route
    router.get('/v1/:id/loans', auth.auth, async (req: any, res: Response, next: NextFunction) => {
      let query;
      ...
      try {
        ...
        const loans = await Db.familyLoan.select(db, query);
        const totaljson: any = await Db.familyLoan.count(db, query);
    ...
    router.put('/v1/:id/loans', auth.auth, async (req: any, res: Response, next: NextFunction) => {
      ...
      try {
        const db = await Db.getSubdb(req.session.db);
        const result = await Db.familyLoan.update(db, query);
    ...
    router.post('/v1/:id/loans/delete', auth.auth, async (req: any, res: Response, next: NextFunction) => {
      if (Object.keys(req.body).length <= 0) {
        return res.status(400).send({ errors: [ErrorUtils.getErrorJson(req.session.user.langTx, 'error_http_body_required_jsondata')] });
      }
      ...
      try {
        ...
        for (let plan of req.body.selectItems) {
          result.push(await Db.familyLoan.del(db, plan.userId));
        }
