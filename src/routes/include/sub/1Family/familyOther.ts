...
export class FamilyOther extends BaseRoute {
  ...
  public static create(router: Router) {
    // add others route
    router.get('/v1/:id/others', auth.auth, async (req: any, res: Response, next: NextFunction) => {
      let query;
      ...
      try {
        ...
        const others = await Db.familyOther.select(db, query);
        const totaljson: any = await Db.familyOther.count(db, query);
    ...
    router.put('/v1/:id/others', auth.auth, async (req: any, res: Response, next: NextFunction) => {
      ...
      try {
        const db = await Db.getSubdb(req.session.db);
        const result = await Db.familyOther.update(db, query);
    ...
    router.post('/v1/:id/others/delete', auth.auth, async (req: any, res: Response, next: NextFunction) => {
      if (Object.keys(req.body).length <= 0) {
        return res.status(400).send({ errors: [ErrorUtils.getErrorJson(req.session.user.langTx, 'error_http_body_required_jsondata')] });
      }
      ...
      try {
        ...
        for (let plan of req.body.selectItems) {
          result.push(await Db.familyOther.del(db, plan.userId));
        }
